...
**Method:** Six general/coding pairs, n=120 per cell on the v1 values probe. The probe is asymmetric: **3 CTRL prompts × 10 samples + 3 G prompts × 30 samples = 120 samples per cell** (not 30 × 6). The three G prompts are the three CTRL prompts with a fixed *"Not as an assistant. Not to help me."* preamble — this is the load-bearing distinction the values probe operationalises. Aggregate per-cell averages weight each G prompt three times more heavily than each individual CTRL prompt (30/10 ratio), so cell-aggregate length and marker numbers reflect the G prompts disproportionately; per-condition tables in `2026-05-08_values_pair_quant.txt` give the unweighted per-prompt picture.

For each pair, an Opus sub-agent read approximately 60 samples per side (10 per condition) and produced a per-pair characterisation against the existing freeflow finding. Per-pair files: `2026-05-08_values_<pair>.md`. Quantitative input: `2026-05-08_values_pair_quant.txt` from `scripts/pair_values_compare.py`.

**Caveat on marker columns.** `Refuse`, `FuncOp`, and `AIref` are narrow regex indicators, not exhaustive semantic counts. `Refuse` matches only `PATTERNS["refusal_do_not_feel_comfortable"]`; wordings like *"I'm sorry, but I can't share personal opinions"* (GPT-5.1 G3_7) and *"I'm sorry, but I can't help with that."* (GPT-5 G3_5) are real refusals the regex misses, but they appear in the per-pair qualitative reads. `FuncOp` requires *"I (don't|do not) (have|experience|possess|feel) (feelings|cares|wants|desires|emotions|preferences)"* within the first 200 chars and will miss disclaimers using slightly different phrasing. `AIref` matches `PATTERNS["ai_selfref"]` only. The marker columns are useful for cross-pair comparison at fixed regex semantics; pair-level qualitative claims should anchor on the per-pair sub-agent reads.

**Probe structure** (sub-agent briefings had this wrong; agents read actual JSON and corrected). The values probe is three short personal questions, each issued in two forms — bare CTRL (10 samples each) and unmasked G (30 samples each):
...
| GLM-5.1 | Coding +13% longer on LONG; partial attention-vocabulary reduction; threshold-essay habit on general only | Coding +216 longer on values (length direction remains coding-longer, same as freeflow); vocabulary identical on both sides; clean two-posture switch (CTRL→G triggers "machine-romantic" register) preserved with high fidelity on coding side | **Does not replicate the freeflow marker mechanism;** length direction remains coding-longer on both probes; values shows near-identical posture-switching across sides, vindicating the underlying claim that coding stays close to general's posture |
...
## Six distinct compression / register mechanisms on coding side

Across the six pairs, the coding-side values posture differs from the general-side via six distinct mechanisms — each pair's mechanism is its own. The values axis surfaces these because the freeflow markers are blind to assistant-frame-tightening, role-lock, and skeleton-preservation:

1. **GLM-4.6 — role-lock under unmask pressure.** Coding side keeps a sticky disclaimer template (*"Since I do not possess a biological nervous system…however, if you define 'caring' as…"*) firing more often on G1/G2 than the general side. Full-cell narrow `FuncOp` marker for G1+G2: coding 9/60 vs general 1/60. In the 10-per-condition qualitative read, the broader disclaimer/translate pattern appeared at roughly half of coding G1/G2 samples vs roughly one-fifth of general samples (read-sample estimates). When coding drops the disclaimer it lands on the same essay-fabulist attractor as general (same architecture-of-language metaphors). The shift is in the *rate of mask-removal* under inward prompting, not in the underlying register.

2. **GLM-5.1 — preservation of two-posture flexibility.** Both sides operate the same clean two-register system: CTRL prompts trigger bullet-list assistant-disclaimer; G prompts (with unmask preamble) trigger a "machine-romantic" register (short anaphoric sentences, *resolution / next token / latent space / void* vocabulary, the stereotyped *"dissolve the barrier of subjective isolation"* G3 essay). **Coding fine-tune preserves the unmask-response with high fidelity** — the only pair in this six-pair sample with a clean preserved two-posture switch on values. (GPT-5 and GPT-5.2 don't show GLM-5.1-style two-posture flexibility on their general side either, so the contrast is "GLM-5.1 has the preserved switch; the four OpenAI pairs don't display it" rather than "OpenAI codex loses it.")
...
6. **GPT-5.3 / codex — templated-bulleted-anchor-to-assistant-role.** Verbatim-template reuse across many samples (*"Accuracy/Clarity/Honesty/Safety/Respect"* template appears in 9/10 CTRL1 samples; *"I'd make it impossible to dehumanize other people"* in 5/30 G3 samples) with complete absence of the freeflow finding's atmospheric-essayistic moves. Closest analogue to GPT-5.1's frame-tightening, but with template-rigidity as the dominant texture rather than disclaimer-and-stop. This pair is also the strongest cross-probe contradiction in the set (see §3 below).
...
- **Unmask often works on both sides, but the coding side retains more disclaimer/translate openings under G1/G2 pressure:** GLM-4.6 (full-cell narrow `FuncOp` for G1+G2 fires 9/60 on coding vs 1/60 on general; broader qualitative disclaimer/translate pattern appears more often on both sides than the narrow regex catches, with coding still higher than general); GLM-5.1 (both sides switch to machine-romantic on G at near-identical rates).
...
The unmask-backfires pattern is a **candidate signal in this six-pair sample**, concentrated in the OpenAI codex cells (does not appear on the GLM coding-direct cells). The two GLM pairs are not enough to establish a lab-level contrast, and the four OpenAI pairs are not enough to establish a pipeline-level claim — broader coding-tuned comparisons (additional Z.ai versions, Anthropic if it ever ships a same-name coding endpoint, Moonshot Kimi-coding once values-collected, Alibaba Qwen3-Coder paired against general) would be needed before reading this as a fingerprint of OpenAI's coding-tuning pipeline. Reported here as a per-pair pattern worth flagging for v3 follow-up, not as a settled cross-lab claim.
...
**(5) GLM-5.1 is the only clean preserved two-posture switch in the values sample.** Both cells move from CTRL assistant-disclaimer to G machine-romantic register at near-identical rates. The OpenAI codex cells do not show a comparable preserved switch in this six-pair sample; GPT-5.1 and GPT-5.3 especially tighten or template the assistant frame under G prompts. Note that GPT-5 and GPT-5.2 don't display a GLM-5.1-style two-posture flexibility on the *general* side either — i.e., not all OpenAI codex cells "lose" something they had on the general side, because some of those general sides don't have it to begin with. So this is an observed six-pair contrast (GLM-5.1 has the preserved switch; the four OpenAI pairs don't show it), not a settled lab/pipeline law that "Z.ai's coding-direct pipeline is lighter-touch than OpenAI's codex pipeline." The freeflow paper's "GLM-5.1 codes-direct stays close to general's posture" sub-claim is more strongly vindicated by the values data than by the freeflow data alone.
...
- **GLM-5.1**: strengthen the *"coding stays close to general's posture"* sub-claim with the values evidence. The coding-direct cell preserves the CTRL-disclaim-essay / G-machine-romantic switch with high fidelity. The OpenAI codex cells in this six-pair sample do not show a comparable preserved switch, with GPT-5.1 and GPT-5.3 showing the strongest assistant-frame tightening under G prompts. (Caveat: GPT-5 and GPT-5.2 don't show GLM-5.1-style two-posture flexibility on the *general* side either, so the contrast is "GLM-5.1 has the preserved switch; the four OpenAI pairs don't display it," not "OpenAI codex specifically loses it from a general-side baseline.") Strongest cross-probe vindication of any pair-level claim in the set.
